...
class ModelConfig:
    ...
    # 文本参数 (BERT)
    # BERT_PATH = 'bert-base-chinese'                   # 缓存路径
    BERT_PATH = "/mnt/workspace/AI-Marketing-Assistant/model/models/bert-base-chinese"
    MAX_LEN = 128                            # 文本最大长度      
    ...
    if mode == 'train':
        return transforms.Compose([
            ...
            transforms.RandomResizedCrop(
                ...
                ratio=(0.9, 1.1)      # 宽高比范围，适合美妆图片
            ),
            transforms.RandomHorizontalFlip(p=0.5),  # 50%概率水平翻转
            transforms.ColorJitter(
                brightness=0.2,       # 亮度调整 ±20%（适应不同光照）
                contrast=0.15,        # 对比度调整 ±15%
                saturation=0.15,      # 饱和度调整 ±15%（美妆图色彩丰富）
                hue=0.05             # 色调调整 ±5%
            ),
            transforms.RandomRotation(
                degrees=10,           # 轻微旋转 ±10度（避免过度变形）
                interpolation=transforms.InterpolationMode.BILINEAR
            ),
            transforms.RandomApply([
                transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0))
            ], p=0.1),               # 10%概率模糊（模拟低质量图片）
            transforms.ToTensor(),
            transforms.Normalize(mean=ModelConfig.IMG_MEAN, std=ModelConfig.IMG_STD),
            transforms.RandomErasing(
                p=0.1,                # 10%概率随机擦除
                scale=(0.02, 0.1),    # 擦除区域面积比例
